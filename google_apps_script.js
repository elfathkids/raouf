...
function buildMatrixSettlementSheet(sheet, data) {
  sheet.clear();
  sheet.setRightToLeft(false); // Left-to-right as in Excel screenshot (OGX -> BATNA -> BLIDA -> ORAN)
  ...
  // Header Row (Row 1)
  var headers = ["", "OGX", "BATNA", "BLIDA", "ORAN", "Total USD"];
  var hRange = sheet.getRange(1, 1, 1, headers.length);
  hRange.setValues([headers]);
  hRange.setBackground("#f1f5f9").setFontWeight("bold").setFontSize(11).setHorizontalAlignment("center");
  ...
  Object.values(supplierMap).forEach(function(sm) {
    ...
    // Display as negative numbers (debts) matching Excel screenshot, or empty if 0
    rows.push([
      sm.name,
      sm.ogx > 0 ? -sm.ogx : "",
      sm.batna > 0 ? -sm.batna : "",
  ...
  var startDataRow = 2;
  if (rows.length > 0) {
    var dataRange = sheet.getRange(startDataRow, 1, rows.length, headers.length);
    dataRange.setValues(rows);
    dataRange.setFontSize(10);
    sheet.getRange(startDataRow, 1, rows.length, 1).setFontWeight("bold").setBackground("#f8fafc");
    sheet.getRange(startDataRow, 2, rows.length, 5).setNumberFormat("#,##0.00;(#,##0.00);\"\"");
  }

  var curRow = startDataRow + rows.length;
  ...
  sheet.getRange(curRow, 1, 1, headers.length).setValues([totInvRow])
    .setFontWeight("bold").setFontSize(10).setBackground("#e2e8f0");
  sheet.getRange(curRow, 2, 1, 5).setNumberFormat("#,##0.00;(#,##0.00);\"0.00\"");

  curRow += 2; // Buffer space

  // 1. Total Payments Row (GREEN)
  var sumOgxPay = 0, sumBatnaPay = 0, sumBlidaPay = 0, sumOranPay = 0, sumTotalPay = 0;
  payments.forEach(function(p) {
    var alloc = p.branchAllocationsUsd || {};
    var ogxP = Number(alloc.ogx) || 0;
    var batnaP = Number(alloc.batna) || 0;
    var blidaP = Number(alloc.blida) || 0;
    var oranP = Number(alloc.oran) || 0;
    var totP = Number(p.totalPaidUsd) || (ogxP + batnaP + blidaP + oranP);
  ...
  var greenRow = [
    "total paiment (إجمالي تسديدات الفروع)",
    sumOgxPay,
    sumBatnaPay,
    sumBlidaPay,
    sumOranPay,
  ...
  var greenRange = sheet.getRange(curRow, 1, 1, headers.length);
  ...
  sheet.getRange(curRow, 2, 1, 5).setNumberFormat("#,##0.00");
  sheet.setRowHeight(curRow, 30);
  curRow++;

  // 2. Net USD Balance Row (ORANGE & RED)
  var netOgxUsd = sumOgxPay - sumOgxInv;
  var netBatnaUsd = sumBatnaPay - sumBatnaInv;
  var netBlidaUsd = sumBlidaPay - sumBlidaInv;
  var netOranUsd = sumOranPay - sumOranInv;
  var netTotalUsd = sumTotalPay - sumTotalInv;

  var orangeRow = [
    "الفارق الصافي USD (Net Balance)",
    netOgxUsd,
    netBatnaUsd,
    netBlidaUsd,
    netOranUsd,
  ...
  var orangeRange = sheet.getRange(curRow, 1, 1, headers.length);
  ...
  sheet.getRange(curRow, 6).setBackground("#ef4444").setFontColor("#ffffff"); // Total Net in RED
  sheet.getRange(curRow, 2, 1, 5).setNumberFormat("#,##0.00;(#,##0.00);\"0.00\"");
  sheet.setRowHeight(curRow, 30);
  curRow++;
  ...
  var rateRange = sheet.getRange(curRow, 1, 1, headers.length);
  rateRange.setValues([rateRow]).setFontWeight("bold").setFontSize(10).setBackground("#f8fafc").setHorizontalAlignment("center");
  sheet.getRange(curRow, 2, 1, 4).setNumberFormat("0.0000000");
  curRow++;

  // 4. Net DZD Equivalent Row (DZD Settlement)
  ...
  var dzdRow = [
    "المعادل الصافي بالدينار (Net DZD)",
    netOgxDzd,
    netBatnaDzd,
    netBlidaDzd,
    netOranDzd,
  ...
  var dzdRange = sheet.getRange(curRow, 1, 1, headers.length);
  dzdRange.setValues([dzdRow]).setFontWeight("black").setFontSize(11).setBackground("#f1f5f9").setHorizontalAlignment("center");
  sheet.getRange(curRow, 2, 1, 5).setNumberFormat('#,##0 "د.ج";(#,##0 "د.ج");"0 د.ج"');
  sheet.setRowHeight(curRow, 30);

  // Borders & Auto sizing
  var allGrid = sheet.getRange(1, 1, curRow, headers.length);
  allGrid.setBorder(true, true, true, true, true, true, "#94a3b8", SpreadsheetApp.BorderStyle.SOLID);
