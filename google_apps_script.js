...
function buildForexSheet(sheet, forex, branches, brokers) {
  ...
  if (rows.length > 0) {
    ...
    sheet.getRange(totIdx, 1, 1, 5).merge().setValue("💶 إجمالي مشتريات الأورو (EUR)").setFontWeight("bold");
    sheet.getRange(totIdx, 6).setFormula('=SUMIF(E2:E' + (totIdx - 1) + ', "EUR", F2:F' + (totIdx - 1) + ')').setNumberFormat('"€"#,##0.00');
    sheet.getRange(totIdx, 8).setFormula('=SUMIF(E2:E' + (totIdx - 1) + ', "EUR", H2:H' + (totIdx - 1) + ')').setNumberFormat('#,##0 "د.ج"');
    ...
    sheet.getRange(totIdx2, 1, 1, 5).merge().setValue("💵 إجمالي مشتريات الدولار (USD)").setFontWeight("bold");
    sheet.getRange(totIdx2, 6).setFormula('=SUMIF(E2:E' + (totIdx - 1) + ', "USD", F2:F' + (totIdx - 1) + ')').setNumberFormat('"$"#,##0.00');
    sheet.getRange(totIdx2, 8).setFormula('=SUMIF(E2:E' + (totIdx - 1) + ', "USD", H2:H' + (totIdx - 1) + ')').setNumberFormat('#,##0 "د.ج"');
    ...
    sheet.getRange(totIdx3, 8).setFormula("=SUM(H2:H" + (totIdx - 1) + ")");
    sheet.getRange(totIdx3, 9).setFormula("=SUM(I2:I" + (totIdx - 1) + ")");
    sheet.getRange(totIdx3, 10).setFormula("=SUM(J2:J" + (totIdx - 1) + ")");
    sheet.getRange(totIdx3, 11).setFormula("=SUM(K2:K" + (totIdx - 1) + ")");
    sheet.getRange(totIdx3, 12).setFormula("=SUM(L2:L" + (totIdx - 1) + ")");
    sheet.getRange(totIdx3, 13).setFormula("=SUM(M2:M" + (totIdx - 1) + ")");
    sheet.getRange(totIdx3, 1, 1, headers.length).setBackground("#e0e7ff").setFontWeight("bold");
    sheet.getRange(totIdx3, 8, 1, 6).setNumberFormat('#,##0 "د.ج"');
  }
...
function buildPaymentsSheet(sheet, payments, suppliers, branches) {
  ...
  (payments || []).forEach(function(p) {
    ...
    var paidCur = p.paidCurrency || "USD";
  ...
  if (rows.length > 0) {
    ...
    // سطر 1: إجمالي المسدد بالأورو EUR
    sheet.getRange(totIdx, 1, 1, 3).merge().setValue("💶 إجمالي المسدد بالأورو (EUR)").setFontWeight("bold");
    sheet.getRange(totIdx, 4).setFormula('=SUMIF(E2:E' + (totIdx - 1) + ', "EUR", D2:D' + (totIdx - 1) + ')').setNumberFormat('"€"#,##0.00');
    sheet.getRange(totIdx, 5).setValue("EUR");
    sheet.getRange(totIdx, 6).setFormula('=SUMIF(E2:E' + (totIdx - 1) + ', "EUR", F2:F' + (totIdx - 1) + ')').setNumberFormat('"$"#,##0.00');
    sheet.getRange(totIdx, 1, 1, headers.length).setBackground("#eff6ff").setFontWeight("bold");

    // سطر 2: إجمالي المسدد بالدولار المباشر USD
    var totIdx2 = totIdx + 1;
    sheet.getRange(totIdx2, 1, 1, 3).merge().setValue("💵 إجمالي المسدد بالدولار المباشر (USD)").setFontWeight("bold");
    sheet.getRange(totIdx2, 4).setFormula('=SUMIF(E2:E' + (totIdx - 1) + ', "USD", D2:D' + (totIdx - 1) + ')').setNumberFormat('"$"#,##0.00');
    ...
    sheet.getRange(totIdx3, 6).setFormula("=SUM(F2:F" + (totIdx - 1) + ")");
    sheet.getRange(totIdx3, 7).setFormula("=SUM(G2:J" + (totIdx - 1) + ")"); // branch shares
    sheet.getRange(totIdx3, 1, 1, headers.length).setBackground("#f3e8ff").setFontWeight("bold");
    sheet.getRange(totIdx3, 6, 1, 5).setNumberFormat('"$"#,##0.00');
  }
