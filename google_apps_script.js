...
function handleTelegramMessage(message) {
  try {
    ...
    // رد توجيهي ذكي
    var defaultReply = "👋 أهلاً بك " + senderName + "! لم أتمكن من استخراج تفاصيل العملية بدقة.\n\n" +
                       "💡 <b>أمثلة واضحة للتسجيل الفوري:</b>\n" +
                       "• <code>هزيت 500000 من باتنة</code>\n" +
                       "• <code>خلصت سويف 2500 دولار</code>\n" +
                       "• <code>فاتورة سيفيل 4000 دولار</code>\n\n" +
                       "📊 أو أرسل <b>/help</b> لعرض خيارات الكشوفات.";
    sendTelegramMessageToChat(chatId, defaultReply);

  } catch(err) {
    Logger.log("Telegram Handler Exception: " + err);
    try {
      sendTelegramMessageToChat(message.chat.id, "⚠️ حدث خطأ أثناء المعالجة: " + err.toString());
    } catch(e) {}
  }
}
