// 🚀 تشغيل فوري وبناء كافة الشيتات بضغطة زر واحدة من المحرر (Run ▶️)
function buildAllSheetsNow() {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var syncTimestamp = Utilities.formatDate(new Date(), "Africa/Algiers", "yyyy-MM-dd HH:mm:ss");

  // جلب آخر بيانات تم إرسالها من الموقع أو استخدام البيانات الشاملة المدمجة
  var payload = null;
  try {
    var saved = PropertiesService.getScriptProperties().getProperty("LATEST_DATA");
    if (saved) {
  ...
  if (!payload || !payload.branches || payload.branches.length === 0) {
    payload = getEmbeddedDefaultData();
  }

  // 1. مصفوفة فواتير المصانع وتصفية حسابات الفروع (المطابقة لملف الإكسل الأصلي 100%)
  var sheetMatrix = getOrCreateSheet(ss, "📊 مصفوفة تصفية الحسابات (Matrix)", 1);
  buildMatrixSettlementSheet(sheetMatrix, payload);

  // 2. لوحة الخزينة والأرصدة العامة
  ...
  Logger.log("تم بناء وتحديث كافة الشيتات بنجاح في: " + syncTimestamp);
}

// دالة بديلة مطابقة لـ testBuild
function testBuild() {
  buildAllSheetsNow();
}
...
function getEmbeddedDefaultData() {
  return {
    ...
    supplierInvoices: [
      { id: 'inv_1', date: '2025-10-01', supplierId: 's_damasquino', invoiceNumber: 'DAM-01', invoiceCurrency: 'USD', totalAmountUsd: 4420.0, branchSharesUsd: { ogx: 1445.0, batna: 1530.0, blida: 0.0, oran: 1445.0 } },
      { id: 'inv_2', date: '2025-10-05', supplierId: 's_civil_demi', invoiceNumber: 'CIV-DEMI-01', invoiceCurrency: 'USD', totalAmountUsd: 14356.25, branchSharesUsd: { ogx: 3053.79, batna: 4139.78, blida: 3472.48, oran: 3690.2 } },
      { id: 'inv_3', date: '2025-10-08', supplierId: 's_civil_summer', invoiceNumber: 'CIV-SUM-01', invoiceCurrency: 'USD', totalAmountUsd: 1625.24, branchSharesUsd: { ogx: 497.4, batna: 582.66, blida: 0.0, oran: 545.18 } },
      { id: 'inv_4', date: '2025-10-10', supplierId: 's_exina', invoiceNumber: 'EXI-01', invoiceCurrency: 'USD', totalAmountUsd: 11238.50, branchSharesUsd: { ogx: 2859.0, batna: 3029.0, blida: 2561.5, oran: 2789.0 } },
      { id: 'inv_5', date: '2025-10-12', supplierId: 's_joi_kids', invoiceNumber: 'JOI-01', invoiceCurrency: 'USD', totalAmountUsd: 2160.0, branchSharesUsd: { ogx: 720.0, batna: 720.0, blida: 0.0, oran: 720.0 } },
      { id: 'inv_6', date: '2025-10-15', supplierId: 's_cikoby', invoiceNumber: 'CIK-01', invoiceCurrency: 'USD', totalAmountUsd: 1481.0, branchSharesUsd: { ogx: 461.0, batna: 535.0, blida: 0.0, oran: 485.0 } },
      { id: 'inv_7', date: '2025-10-16', supplierId: 's_pengim', invoiceNumber: 'PEN-01', invoiceCurrency: 'USD', totalAmountUsd: 3384.0, branchSharesUsd: { ogx: 1128.0, batna: 1128.0, blida: 0.0, oran: 1128.0 } },
      { id: 'inv_8', date: '2025-10-18', supplierId: 's_mutlu_2', invoiceNumber: 'MUT-02', invoiceCurrency: 'USD', totalAmountUsd: 2160.0, branchSharesUsd: { ogx: 652.0, batna: 827.0, blida: 0.0, oran: 681.0 } },
      { id: 'inv_9', date: '2025-10-20', supplierId: 's_mutlu_pdf1', invoiceNumber: 'MUT-PDF1', invoiceCurrency: 'USD', totalAmountUsd: 9090.0, branchSharesUsd: { ogx: 2612.0, batna: 2819.0, blida: 0.0, oran: 3659.0 } },
      { id: 'inv_10', date: '2025-10-22', supplierId: 's_mutlu_pdf2', invoiceNumber: 'MUT-PDF2', invoiceCurrency: 'USD', totalAmountUsd: 1513.0, branchSharesUsd: { ogx: 471.0, batna: 521.0, blida: 0.0, oran: 521.0 } },
      { id: 'inv_11', date: '2025-10-25', supplierId: 's_clementine', invoiceNumber: 'CLE-01', invoiceCurrency: 'USD', totalAmountUsd: 11858.74, branchSharesUsd: { ogx: 3787.16, batna: 3562.32, blida: 1340.04, oran: 3169.22 } },
      { id: 'inv_12', date: '2025-10-28', supplierId: 's_elsima', invoiceNumber: 'ELS-01', invoiceCurrency: 'USD', totalAmountUsd: 12759.90, branchSharesUsd: { ogx: 4257.3, batna: 4491.3, blida: 0.0, oran: 4011.3 } },
      { id: 'inv_13', date: '2025-10-30', supplierId: 's_dalex', invoiceNumber: 'DAL-01', invoiceCurrency: 'USD', totalAmountUsd: 4105.75, branchSharesUsd: { ogx: 1323.0, batna: 1426.5, blida: 0.0, oran: 1356.25 } },
      { id: 'inv_14', date: '2025-11-02', supplierId: 's_soydan', invoiceNumber: 'SOY-01', invoiceCurrency: 'USD', totalAmountUsd: 23790.0, branchSharesUsd: { ogx: 7610.0, batna: 8600.0, blida: 0.0, oran: 7580.0 } },
      { id: 'inv_15', date: '2025-11-05', supplierId: 's_bbs', invoiceNumber: 'BBS-01', invoiceCurrency: 'USD', totalAmountUsd: 2200.0, branchSharesUsd: { ogx: 650.0, batna: 800.0, blida: 100.0, oran: 650.0 } },
      { id: 'inv_16', date: '2025-11-08', supplierId: 's_kocak', invoiceNumber: 'KOC-01', invoiceCurrency: 'USD', totalAmountUsd: 4500.0, branchSharesUsd: { ogx: 1500.0, batna: 1500.0, blida: 0.0, oran: 1500.0 } },
      { id: 'inv_17', date: '2025-11-10', supplierId: 's_mdm1', invoiceNumber: 'MDM-01', invoiceCurrency: 'USD', totalAmountUsd: 6360.55, branchSharesUsd: { ogx: 1249.5, batna: 2771.85, blida: 0.0, oran: 2339.2 } },
      { id: 'inv_18', date: '2025-11-12', supplierId: 's_mdm2', invoiceNumber: 'MDM-02', invoiceCurrency: 'USD', totalAmountUsd: 1800.0, branchSharesUsd: { ogx: 0.0, batna: 826.0, blida: 0.0, oran: 974.0 } },
      { id: 'inv_19', date: '2025-11-15', supplierId: 's_himms', invoiceNumber: 'HIM-01', invoiceCurrency: 'USD', totalAmountUsd: 2320.0, branchSharesUsd: { ogx: 0.0, batna: 0.0, blida: 0.0, oran: 2320.0 } }
    ],
    supplierPayments: [],
    settings: {
      usdToDzdReferenceRate: 250.9677161,
...
function buildDashboardSheet(sheet, data, syncTimestamp) {
  ...
  sheet.getRange("B5").setValue(totDzdUsed).setNumberFormat("#,##0 د.ج");
