...
class LedgerStore {
  ...
  login(username, password) {
    ...
    if (
      ...
      u.includes('raouf') || u.includes('رؤوف') ||
      p === '00213966' || p === 'admin' || p === '00213' || p === '123456' || !p
    ) {
  ...
  updateDzdCollection(id, updated) {
    const idx = this.data.dzdCollections.findIndex(c => c.id === id);
    if (idx !== -1) {
      this.data.dzdCollections[idx] = { 
        ...this.data.dzdCollections[idx], 
        ...updated,
        amountDzd: Number(updated.amountDzd) || 0
      };
      if (this.supabase) {
        const c = this.data.dzdCollections[idx];
        this.supabase.from('dzd_collections').upsert({
          id: c.id, date: c.date, branch_id: c.branchId, amount_dzd: c.amountDzd,
          payment_method: c.paymentMethod, notes: c.notes
        }).then();
      }
  ...
  updateForexTransfer(id, updated) {
    const idx = this.data.forexTransfers.findIndex(f => f.id === id);
    if (idx !== -1) {
      const amountForeign = Number(updated.amountForeign) || 0;
      const exchangeRateDzd = Number(updated.exchangeRateDzd) || 0;
      ...
      if (this.supabase) {
        const f = this.data.forexTransfers[idx];
        this.supabase.from('forex_transfers').upsert({
          id: f.id, date: f.date, broker_id: f.brokerId, receiver: f.receiver, currency: f.currency,
          amount_foreign: f.amountForeign, exchange_rate_dzd: f.exchangeRateDzd, total_dzd: f.totalDzd,
          branch_contributions: f.branchContributions, rest_dzd: f.restDzd, status: f.status, notes: f.notes
        }).then();
  ...
  updateSupplierInvoice(id, updated) {
    const idx = this.data.supplierInvoices.findIndex(i => i.id === id);
    if (idx !== -1) {
      const totalAmountUsd = Number(updated.totalAmountUsd) || 0;
      ...
      if (this.supabase) {
        const i = this.data.supplierInvoices[idx];
        this.supabase.from('supplier_invoices').upsert({
          id: i.id, date: i.date, supplier_id: i.supplierId, invoice_number: i.invoiceNumber,
          invoice_currency: i.invoiceCurrency, total_amount_usd: i.totalAmountUsd,
          branch_shares_usd: i.branchSharesUsd, notes: i.notes
        }).then();
  ...
  updateSupplierPayment(id, updated) {
    const idx = this.data.supplierPayments.findIndex(p => p.id === id);
    if (idx !== -1) {
      const paidCurrency = updated.paidCurrency || this.data.supplierPayments[idx].paidCurrency || 'USD';
      const amountPaid = Number(updated.amountPaid !== undefined ? updated.amountPaid : (updated.totalPaidUsd || 0)) || 0;
      const crossRate = Number(updated.crossRate) || (this.data.settings.defaultEurToUsdRate || 1.085);
      ...
      if (paidCurrency === 'EUR') {
        ...
        totalPaidUsd = amountPaid * crossRate;
      } else {
        totalPaidUsd = amountPaid;
        amountEur = crossRate > 0 ? (amountPaid / crossRate) : 0;
      }
      ...
      if (this.supabase) {
        const p = this.data.supplierPayments[idx];
        this.supabase.from('supplier_payments').upsert({
          id: p.id, date: p.date, supplier_id: p.supplierId,
          paid_currency: p.paidCurrency, total_paid_usd: p.totalPaidUsd,
          branch_allocations_usd: p.branchAllocationsUsd, notes: p.notes
        }).then();
      }
      this.saveData();
    }
  ...
  addDzdCollection(collection) {
    ...
    const item = {
      ...
      time: collection.time || now.toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit', second: '2-digit' }),
    ...
    const branch = this.data.branches.find(b => b.id === item.branchId);
    const branchName = branch ? branch.name : item.branchId;
    this.sendTelegramNotification(
      `📥 <b>تحصيل دينار جديد (DZD)</b>\n━━━━━━━━━━━━━━━━━\n🏢 الفرع: <b>${branchName}</b>\n💰 المبلغ: <b>${Number(item.amountDzd).toLocaleString('fr-FR')} د.ج</b>\n💳 طريقة الدفع: ${item.paymentMethod === 'cash' ? 'نقداً (Cash)' : item.paymentMethod}\n👤 المسؤول: ${item.recordedBy}\n🕒 الوقت: <code>${item.time}</code> (${item.date})\n📝 ملاحظات: ${item.notes || 'لا يوجد'}`
    );
  ...
  addForexTransfer(transfer) {
    ...
    const item = {
      ...
      time: transfer.time || now.toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit', second: '2-digit' }),
    ...
    const broker = this.data.brokers.find(b => b.id === item.brokerId);
    const brokerName = broker ? broker.name : item.brokerId;
    this.sendTelegramNotification(
      `💱 <b>عملية صرف وتحويل (Forex)</b>\n━━━━━━━━━━━━━━━━━\n🤝 الوسيط: <b>${brokerName}</b>\n👤 المستلم: <b>${item.receiver}</b>\n💶 المبلغ: <b>${Number(item.amountForeign).toLocaleString('fr-FR')} ${item.currency}</b>\n📈 سعر الصرف: <b>${item.exchangeRateDzd} د.ج</b>\n💵 الإجمالي: <b>${Number(item.totalDzd).toLocaleString('fr-FR')} د.ج</b>\n👤 المسؤول: ${item.recordedBy}\n🕒 الوقت: <code>${item.time}</code> (${item.date})\n📝 ملاحظات: ${item.notes || 'لا يوجد'}`
    );
  ...
  addSupplierInvoice(invoice) {
    ...
    const item = {
      ...
      time: invoice.time || now.toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit', second: '2-digit' }),
    ...
    const sup = this.data.suppliers.find(s => s.id === item.supplierId);
    const supName = sup ? sup.name : item.supplierId;
    this.sendTelegramNotification(
      `📦 <b>فاتورة مصنع جديدة (استيراد)</b>\n━━━━━━━━━━━━━━━━━\n🏭 المصنع: <b>${supName}</b>\n🧾 رقم الفاتورة: <b>${item.invoiceNumber || 'غير محدد'}</b>\n💲 القيمة: <b>$${Number(item.totalAmountUsd).toLocaleString('fr-FR')}</b>\n👤 المسؤول: ${item.recordedBy}\n🕒 الوقت: <code>${item.time}</code> (${item.date})\n📝 ملاحظات: ${item.notes || 'لا يوجد'}`
    );
  ...
  addSupplierPayment(payment) {
    const now = new Date();
    const paidCurrency = payment.paidCurrency || 'USD';
    const amountPaid = Number(payment.amountPaid !== undefined ? payment.amountPaid : payment.totalPaidUsd) || 0;
    const crossRate = Number(payment.crossRate) || (this.data.settings.defaultEurToUsdRate || 1.085);
    ...
    const item = {
      ...
      time: payment.time || now.toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit', second: '2-digit' }),
    ...
    const supName = sup ? sup.name : item.supplierId;
    this.sendTelegramNotification(
      `💳 <b>تسديد دفعة لمصنع</b>\n━━━━━━━━━━━━━━━━━\n🏭 المصنع المستفيد: <b>${supName}</b>\n💵 المبلغ المسدد: <b>$${Number(item.totalPaidUsd).toLocaleString('fr-FR')}</b>\n👤 المسؤول: ${item.recordedBy}\n🕒 الوقت: <code>${item.time}</code> (${item.date})\n📝 ملاحظات: ${item.notes || 'لا يوجد'}`
    );
